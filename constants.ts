...
export const MOCK_SHIFTS: Shift[] = [
  ...
  },
  {
    ...
    roundingMinutes: 30,
    workDays: [1, 2, 3, 4, 5, 6], // Mon-Sat
    isSaturdayHalfDay: true // Sat ends at 12:00
  }
];
...
// Initialize some logs for the current month
...
MOCK_EMPLOYEES.forEach(emp => {
    for(let i=1; i<=daysInMonth; i++) {
        ...
        // Find shift to see if working day
        ...
        // 80% chance of perfect attendance
        if (rand < 0.8) {
            let start = shift.startTime;
            let end = shift.endTime;
            
            // Adjust for Sat half day
            if (dayOfWeek === 6 && shift.isSaturdayHalfDay) {
                end = '12:00';
            }

            // Add randomness (-5 to +5 mins)
            // Skipping complex time math for mock, just string manipulation approximation
